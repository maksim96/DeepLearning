...
def compute_loss(logits, weights, y, regularizer):
    ...
    with tf.name_scope('Loss'):
        ...
        if (regularizer == None):
            ...
            loss = cost + weight_decay_l2 * (
                    tf.nn.l2_loss(weights['h1']) + tf.nn.l2_loss(weights['h2']) + tf.nn.l2_loss(weights['out']))
        elif (regularizer == "L1"):
            loss = cost + weight_decay_l1 * (
                    tf.math.abs(weights['h1']) + tf.math.abs(weights['h2']) + tf.math.abs(weights['out']))
...
def train_network(optimizer='sgd', regularizer=None):
    ...
    for i in range(10000):
        ...
        if (i % 100) == 0:  # Record summaries ( loss etc) and test-set accuracy
            summary, acc = cal_accuracy(pred, y)
